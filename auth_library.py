...
def get_fmcauth_token():
    ...
    resp = requests.post(url, auth=HTTPBasicAuth(handlers['fmc']['fmc_username'], handlers['fmc']['fmc_password']),
                         verify=False)
...
# Token Auth with ACI
def get_aci_token():
    url = handlers['aci']['apic_baseurl']+handlers['aci']['apic_loginpath']
    #print(url)
    body = {'aaauser': {"atrributes": handlers['aci']['apic_username'], "pwd": handlers['aci']['apic_password']}}
    #print(body)
